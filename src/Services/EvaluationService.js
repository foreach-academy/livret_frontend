...
class EvaluationServices {
    ...
    static addEvaluation(evaluation) {
        return axios.post(`${URL}/evaluation`, evaluation); // Envoie une requête POST avec les données de l'évaluation
    }

    // Méthode pour modifier une évaluation existante en fonction de son ID
    static editEvaluation(evaluationId, evaluation) {
        return axios.patch(`${URL}/evaluation/${evaluationId}`, evaluation); // Envoie une requête PATCH pour mettre à jour l'évaluation
    }

    // Méthode pour récupérer tous les types d'évaluation disponibles
    static getAllEvaluationTypes() {
        return axios.get(`${URL}/evaluation-type`); // Envoie une requête GET pour obtenir tous les types d'évaluation
    }

    // Méthode pour associer un type d'évaluation spécifique à un module
    static addEvaluationTypeToModule(evaluationType) {
        return axios.post(`${URL}/evaluation-type`, evaluationType); // Envoie une requête POST pour ajouter un type d'évaluation à un module
    }

    // Méthode pour récupérer tous les résultats d'évaluation possibles
    static getAllEvaluationResultats() {
        return axios.get(`${URL}/evaluation-resultat`); // Envoie une requête GET pour obtenir tous les résultats d'évaluation
    }
    ...
    static getEvaluationTypeByModuleId(moduleId) {
        return axios.get(`${URL}/evaluation-type/${moduleId}`); // Envoie une requête GET pour obtenir les types d'évaluation du module
    }

    // Méthode pour obtenir les détails d'un module spécifique via son ID
    static getModuleById(moduleId) {
        return axios.get(`${URL}/module/${moduleId}`); // Envoie une requête GET pour obtenir les informations du module
    }
